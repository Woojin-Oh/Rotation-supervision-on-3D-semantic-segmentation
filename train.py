...
class Trainer(object):
    ...
    def _initDataloader(self):
        ...
        if self.settings.dataset == 'nuScenes':
            ...
            self.mapped_cls_name = trainset.mapped_cls_name #customize label 이름
            self.ignore_class = [0]
    ...
    def run_with_kpconv(self, epoch, mode='Train', print_results=False, save_results_path=None):
        ...
        for i, batch_dict in enumerate(dataloader):
            ...
            # 2D inputs
            input_feature = batch_dict['input2d'].cuda(non_blocking=True) #range 각 grid의 feature들(range, z,x,y,intensity), [B, 5, H,W]
            ...
            if mode == 'Train':
                ...
                with torch.no_grad():
                    assert input_feature.shape[0] == 1 # validation batch size has to be 1 [1, 5, H, W]

                    # Validation
                    im_meta = dict(flip=False) #flip: 변수
                    with torch.cuda.amp.autocast(self.fp16_scaler is not None):
                        output_features2d = inference(
                            ...
                            [im_meta],
                            ori_shape=input_feature.shape[2:4],
                            window_size=self.settings.window_size, #[32,384]
                            window_stride=self.settings.window_stride, #[32,256]
                            batch_size=input_feature.shape[0],
                            use_kpconv=True)
                    ...
                    # Loss calculation
                    total_loss, loss_lovasz, loss_focal = self.compute_losses( #output3d: [N,num_classes, 1, 1](리튼피셜), labels3d : [N,1,1]
                        output3d, output3d_softmax, labels3d, mask_3d)
