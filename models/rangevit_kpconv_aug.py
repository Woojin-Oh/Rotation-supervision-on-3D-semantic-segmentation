...
class RangeViT_KPConv(nn.Module):
    ...
    def forward(self, im, px, py, pxyz, pknn, num_points, is_train, window_size, window_stride, im_meta):
        #feats = self.forward_2d_features(im)
        #여기에 inference 부분 추가
        if is_train:
            output_features2d = inference_train(
        ...
        mask3d = self.kpclassifier(
                    output_features2d, px, py, pxyz, pknn, num_points)
