...
from Proto.lib.utils.tools.logger import Logger as Log

from Proto.lib.utils.distributed import get_rank, is_distributed
from Proto.lib.loss.loss_helper import FSAuxRMILoss, FSCELoss
from Proto.lib.loss.loss_proto import PixelPrototypeCELoss
...
class Trainer(object):
    def __init__(self, settings: Option, model: nn.Module, recorder=None):
        ...
        if tools.is_dist_avail_and_initialized():
            #print('self.model.rangevit.prototypes.to(device): ', self.model.rangevit.prototypes.get_device())
        ...
        #proto
        alpha = [0. , 0.46204817, 0.9830099,  0.95660925, 0.8380046,  0.8554145, 0.96306926, 0.98795223, 1. ,0.2608562,  0.6066046,  0.30076912,
                 0.7745067,  0.31107822, 0.3904502,  0.22624421, 0.72233045, 0.3801905, 0.8093845, 0.93563026]
        loss = PixelPrototypeCELoss(2, alpha)

        self.pixel_loss = self._parallel(loss)       

        #print('is distributed: ', is_distributed)
        if is_distributed():
            #print('use distribution')
            device = torch.device('cuda:{}'.format(get_rank()))
            self.pixel_loss.to(device) 
            #print('self.model.rangevit.prototypes.to(device): ', self.model.rangevit.prototypes.to(device))
            #print('here')
    ...
    def _parallel(self, loss):
        ...
        if self.configer.get('network', 'loss_balance') and len(self.configer.get('gpu')) > 1:
            ...
            from Proto.lib.extensions.parallel.data_parallel import DataParallelCriterion
    ...
    def run(self, epoch, mode='Train', print_results=False, save_results_path=None):
        if self.settings.use_kpconv:
            ...
            return self.run_with_kpconv_proto(
    ...
    def run_with_kpconv(self, epoch, mode='Train', print_results=False, save_results_path=None):
        ...
        for i, batch_dict in enumerate(dataloader):
            ...
            if mode == 'Train':
                with torch.cuda.amp.autocast(self.fp16_scaler is not None):
                    print('input_features shape: ',input_feature.shape)
            ...
            if (mode == 'Validation' and save_results_path is not None):
                ...
                if self.settings.dataset == 'nuScenes':
                    ...
                    #print(dir(nu_dataset))
                    lidar_token = nu_dataset.lidar_filenames[index]
                    #lidar_token = nu_dataset.token_list[index]
                    #lidar_token = nu_dataset[index]
    ...
    # Method for training and validation when using the KPConv layer
    def run_with_kpconv_proto(self, epoch, mode='Train', print_results=False, save_results_path=None):
        if mode == 'Train':
            dataloader = self.train_loader
            self.model.train()
        ...
        for i, batch_dict in enumerate(dataloader):
            ...
            if mode == 'Train':
                with torch.cuda.amp.autocast(self.fp16_scaler is not None):
                    #print('input_features shape: ',input_feature.shape) #[4, 5, 64, 384]
                    is_train = True

                    outputs3d = self.model(input_feature, px, py, pxyz, knns, num_points, labels3d, is_train)
                    #output3d : seg, logits, target의 dict

                    #print('outputs3d keys: ', outputs3d.keys())
                    total_loss = self.pixel_loss(outputs3d, labels3d, mask_3d)
                ...
                with torch.no_grad():
                    ...
                    is_train = False
                    with torch.cuda.amp.autocast(self.fp16_scaler is not None):
                        outputs3d = self.model(input_feature, px, py, pxyz, knns, num_points, labels3d, is_train, im_meta, self.settings.window_size, self.settings.window_stride)
            ...
            #loss_focal_mean = loss_focal.mean()
            with torch.no_grad():
                argmax3d = outputs3d['seg'].argmax(dim=1)
                self.metrics.addBatch(argmax3d, labels3d) # 3D predictions
