...
class Experiment(object):
    ...
    def _loadCheckpoint(self):
        if self.settings.checkpoint is not None:
            ...
            device = self.model.rangevit.prototypes.get_device()
            print('checkpoint data keys: ', checkpoint_data.keys())
            self.model.rangevit.prototypes = checkpoint_data['prototype'].to(device)
            #print('prototypes: ', self.model.rangevit.prototypes)
        ...
        for epoch in range(self.epoch_start, self.settings.n_epochs):
                    ...
                    for k, v in val_result.items():
                        if v >= best_val_result[k]:
                            ...
                            checkpoint_data = {
                                ...
                                'epoch': epoch,
                                'prototype': self.model.rangevit.prototypes,
                                k: v,
                            }
            ...
            if self.recorder is not None:
                ...
                if self.trainer.fp16_scaler is not None:
                    checkpoint_data['fp16_scaler'] = self.trainer.fp16_scaler.state_dict()
                torch.save(checkpoint_data, saved_path)
                #print('dir RangeVit.rangevit: ', dir(self.model.rangevit))
                #print('prototype: ', self.model.rangevit.prototypes.shape)
