...
class RangeViewLoader(Dataset):
    def __init__(self, dataset, config, data_len=-1, is_train=True, return_uproj=False, use_kpconv=False):
        ...
        if self.is_train:
            ...
            augment_params.setRotationParams(
                ...
                p_rot_yaw=1, rot_yawmin=270, rot_yawmax=270) #yaw: z축 회전
            self.augmentor = augmentor.Augmentor(augment_params)
            '''
        ...
        # Image augmentations , 여기서 train시 random crop
        '''
        if self.is_train:
    ...
    def get_item_for_kpconv(self, index):
        ...
        def process_pointcloud(self, pointcloud, sem_label, index, points_xyz):
            ...
            proj_sem_label_tensor = proj_sem_label_tensor * proj_mask_tensor.float() #sem_label이 유효한 곳만 남기고, tensor로 변환

            proj_range_tensor = torch.from_numpy(proj_range)
            proj_xyz_tensor = torch.from_numpy(proj_pointcloud[..., :3]) #proj_pointcloud [H,W,3]
            proj_intensity_tensor = torch.from_numpy(proj_pointcloud[..., 3])
            proj_intensity_tensor = proj_intensity_tensor.ne(-1).float() * proj_intensity_tensor #ne(-1) : 원소가 -1이면 false, -1이 아니면 True 반환, .float을 통해 true면 1.0, false 0.0
            proj_feature_tensor = torch.cat(
                [proj_range_tensor.unsqueeze(0), proj_xyz_tensor.permute(2, 0, 1), proj_intensity_tensor.unsqueeze(0)], 0) #permute: [H,W,3]-> [3,H,W]
            #feature_tensor: range, x, y, z, intensity (기존 H*W 모양을 unsqueeze를 통해 [1, H, W]로 바꿈)
            #proj_feature_tensor: [5,H,W]

            proj_feature_tensor = (proj_feature_tensor - self.proj_img_mean[:, None, None]) / self.proj_img_stds[:, None, None] #각 feature 마다 정규화
            ...
            _, knns = tree.query(points_xyz, k=7) #knns: 각 점에서 가장 가까운 7개의 점 index 배열 변환, [n, 7]

            output = {
                'input2d': proj_tensor[:5], #feature_tensor
        ...
        #if self.is_train and (self.scan_proj is False): 기존
        '''
        if self.is_train and (self.scan_proj is False):
            pointcloud_w = self.augmentor.doAugmentation_weak(pointcloud)  # n, 4(x,y,z,intensity)
            pointcloud_s = self.augmentor.doAugmentation_strong(pointcloud_w)
            output_s = process_pointcloud(self, pointcloud_w, sem_label)
            output_w = process_pointcloud(self, pointcloud_s, sem_label)

            merged_output = {}
            for key, value in output_w.items():
                merged_output[key] = value

            for key, value in output_s.items():
                merged_output[key + '_s'] = value
        '''
        if self.is_train:
            if self.scan_proj is False:
                pointcloud = self.augmentor.doAugmentation_weak(pointcloud)
                output = process_pointcloud(self, pointcloud, sem_label, index, points_xyz)
                return output
            else:
                output = process_pointcloud(self,pointcloud, sem_label, index, points_xyz)


                pointcloud_rotate = self.augmentor.doAugmentation_strong(pointcloud.copy())
                points_xyz = pointcloud_rotate[:, :3]
                output_rotate = process_pointcloud(self, pointcloud_rotate, sem_label, index, points_xyz)
        
                #print('is aug?: ',output['points_xyz'] == output_rotate['points_xyz'] )
                #print('aug before: ', output['points_xyz'][0])
                #print('aug after: ', output_rotate['points_xyz'][0])
                #print('aug before: ',pointcloud[0])
                #print('aug after: ', pointcloud_rotate[0])

                for key, value in output_rotate.items():
                    output[key + '_s'] = value
...
def custom_collate_kpconv_fn(list_data):
    ...
    for key in list_data[0].keys():
        if key in ('input2d', 'mask2d', 'label2d', 'input2d_s', 'mask2d_s', 'label2d_s'):
            output[key] = torch.stack([v[key] for v in list_data], dim=0) #ex) 2개의 [3,4] -> [2,3,4]
        elif key in ('px', 'py', 'points_xyz', 'knns', 'labels', 'px_s', 'py_s', 'points_xyz_s', 'knns_s', 'labels_s'):
            output[key] = torch.cat([v[key] for v in list_data], dim=0) #ex) 2개의 [3,4] -> [6,4]
        elif key in ('num_points', 'index', 'num_points_s', 'index_s'):
