...
#point cloud augmentation, projection 정의
class RangeViewLoader(Dataset):
    ...
    #실제로 dataloader 부분에서 data 불러오는 부분
    def get_item_for_kpconv(self, index):
        ...
        if (self.scan_proj is False):
            augmented_pointcloud = self.augmentor.doAugmentation(pointcloud.copy())  # n, 4(x,y,z,intensity)

        output_augmented = process_pointcloud(self, augmented_pointcloud)

        if not self.is_train:
            output_original = process_pointcloud(self, pointcloud)

            # (선택 사항) 결과를 하나로 합치려면 아래와 같이 처리할 수 있습니다.
            output_combined = {key: (value, output_augmented[key])
                            for key, value in output_original.items()}
    ...
    #이게 호출이 됨, use_kpnocv가 true면 self.get_itme_for_kpconv 실행
