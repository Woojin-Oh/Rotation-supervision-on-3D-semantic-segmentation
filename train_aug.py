...
class Trainer(object):
    def __init__(self, settings: Option, model: nn.Module, recorder=None):
        ...
        self.metrics = utils.metrics.IOUEval(
            ...
            ignore=self.ignore_class, is_distributed=self.settings.distributed) #ignore_class = [0], distributed = false
        self.metrics.reset()
    ...
    def _initDataloader(self):
        ...
        if tools.is_dist_avail_and_initialized():
            ...
            #train_sampler = torch.utils.data.distributed.DistributedSampler(trainset, shuffle=False)
    ...
    def compute_losses_aug(self, output, output_softmax, label, mask, output_softmax_s):
        loss_lovasz = self.criterion['lovasz'](output_softmax, label)
        loss_focal = self.criterion['focal_loss'](output_softmax, label, mask=mask)
        argmax3d = output_softmax.argmax(dim=1)
        loss_aug = self.criterion['focal_loss'](output_softmax_s, argmax3d, mask = mask)
        total_loss = loss_focal + loss_lovasz + loss_aug
        return total_loss, loss_lovasz, loss_focal, loss_aug
    ...
    def run_with_kpconv(self, epoch, mode='Train', print_results=False, save_results_path=None):
        ...
        for i, batch_dict in enumerate(dataloader):
            ...
            # 2D inputs
            #input_feature = batch_dict['input2d'][0].cuda(non_blocking=True)
            input_feature = batch_dict['input2d'].cuda(non_blocking=True)
            assert self.settings.in_channels == 5

            # 3D inputs
            #print('labels: ', batch_dict['labels'].shape)
            ...
            #print('labels3d: ', labels3d.shape)
            #custom

            val_labels = batch_dict['labels'].cuda(non_blocking=True)
            ...
            if mode == 'Train':
                ...
                #rotation_aug
                input_feature_s = batch_dict['input2d_s'].cuda(non_blocking=True)
                py_s = batch_dict['py_s'].cuda(non_blocking=True)
                px_s = batch_dict['px_s'].cuda(non_blocking=True)
                pxyz_s = batch_dict['points_xyz_s'].cuda(non_blocking=True)
                knns_s = batch_dict['knns_s'].cuda(non_blocking=True)
                num_points_s = batch_dict['num_points_s']

                #print('pxyz: ', pxyz[0])
                #print('pxyz_s: ', pxyz_s[0])
                #print('train pxyz-pxyz_s: ', pxyz == pxyz_s)
                #print('train intput-input_s: ', input_feature == input_feature_s)

                #print('px shape: ', px.shape)

                input_features_comb =  torch.cat((input_feature, input_feature_s), dim=0)
                px_comb = torch.cat((px, px_s), dim=0)
                py_comb = torch.cat((py, py_s), dim=0)
                pxyz_comb = torch.cat((pxyz, pxyz_s), dim=0)
                knns_comb = torch.cat((knns, knns_s), dim=0)
                num_points_comb = torch.cat((num_points, num_points_s), dim=0)

                #print('input_features_com shape: ', input_features_comb.shape)
                #print('px shape: ', px_comb.shape)
                #print('py shape: ', py_comb.shape)
                #print('pxyz shape: ', pxyz_comb.shape)
                #print('knn shape: ', knns_comb.shape)
                #print('num_points shape: ', num_points_comb.shape)
                ...
                with torch.cuda.amp.autocast(self.fp16_scaler is not None):
                    ...
                    #batch도 고려
                    is_train = True
                    #output3d = self.model(input_feature, px, py, pxyz, knns, num_points, is_train, 
                    #        self.settings.window_size, self.settings.window_stride, im_meta)
                    output3d = self.model(input_features_comb, px_comb, py_comb, pxyz_comb, knns_comb, num_points_comb, is_train, 
                            self.settings.window_size, self.settings.window_stride, im_meta)

                    output3d_softmax = F.softmax(output3d, dim=1)

                    ori_num_points = torch.sum(num_points_comb[:int(len(num_points_comb)/2)])
                    #aug_num_points = torch.sum(num_points_comb[len(num_points_comb)/2:])
                    output3d_softmax_s = output3d_softmax[ori_num_points:]
                    output3d_softmax = output3d_softmax[:ori_num_points]
                    output3d = output3d[:ori_num_points]

                    

                    #output3d_s = self.model(input_feature_s, px_s, py_s, pxyz_s, knns_s, num_points_s, is_train, 
                    #        self.settings.window_size, self.settings.window_stride, im_meta)
                    

                    #output3d_softmax_s = F.softmax(output3d_s, dim=1)
                    ...
                    total_loss, loss_lovasz, loss_focal, loss_aug = self.compute_losses_aug(
                        output3d, output3d_softmax, labels3d, mask_3d, output3d_softmax_s)
                ...
                with torch.no_grad():
                    ...
                    assert input_feature.shape[0] == 1 # validation batch size has to be 1
                    #assert input_feature.shape[0] == 2 # orig + aug
                    
                    # Validation
                    im_meta = dict(flip=False)
                    is_train = False
                    with torch.cuda.amp.autocast(self.fp16_scaler is not None):
                        output3d = model_without_ddp(input_feature, px, py, pxyz, knns, num_points, is_train, 
                            self.settings.window_size, self.settings.window_stride, im_meta)
                        ...
                        #if i == 0:
                        #    print('output_feature2d before',output_features2d.shape)
                        output_features2d = output_features2d.unsqueeze(0) # [C, H, W] ==> [1, C, H, W], [256,32,2048]

                        # Apply KPConv layer
                        output3d = model_without_ddp.rangevit.kpclassifier(
                            output_features2d, px, py, pxyz, knns, num_points)
                        #if i == 0:
                        #    print('output3d: ',output3d.shape) #[32752(num_point), 17, 1, 1] sample마다 개수 다름
                        '''
            ...
            # Measure IoU and record loss
            loss = total_loss.mean()
            loss_lovasz_mean = loss_lovasz.mean()
            loss_focal_mean = loss_focal.mean()
            #loss_aug_mean = loss_aug.mean()
            with torch.no_grad():
            ...
            if (mode == 'Validation' and save_results_path is not None):
                ...
                if self.settings.dataset == 'nuScenes':
                    ...
                    lidar_token = nu_dataset.lidar_filenames[index][:-8]
                    ...
                    print('pred_result path', pred_result_path)
                    pred_np_origin.tofile(pred_result_path)
                    print('saved')

                    #rotation 확인하기 위해 point 저장
                    if frame_id == '000000':
                        pxyz_np = pxyz.cpu().numpy()
                        point_org = pxyz_np.reshape((-1)).astype(np.float32)
                        point_path = os.path.join(pred_path,'{}.bin'.format(frame_id))
                        point_org.tofile(point_path)
            ...
            if (i % self.settings.log_frequency == 0) or (i == total_iter-1):
                with torch.no_grad():
                    mean_iou, _, mean_acc, _ = self.metrics.getIoUnAcc() #누적 IoU, ACC (addbatch를 통해 한 matrix에 계속 더함)
                if self.recorder is not None:
                    for g in self.optimizer.param_groups:
                        lr = g['lr']
                        break
                    ...
                    if mode == 'Train':
                        log_str += 'LR {} Loss {:0.4f} Loss_lov {:0.4f} Loss_foc {:0.4f} Loss_aug {:0.4f} Acc {:0.4f} IOU {:0.4F} '.format(
                            lr, loss.item(),loss_lovasz_mean.item(),loss_focal_mean.item(),loss_aug_mean.item(), mean_acc.item(), mean_iou.item())
                    else:
                        log_str += 'LR {} Loss {:0.4f} Loss_lov {:0.4f} Loss_foc {:0.4f} Acc {:0.4f} IOU {:0.4F} '.format(
                            lr, loss.item(),loss_lovasz_mean.item(),loss_focal_mean.item(), mean_acc.item(), mean_iou.item())
        ...
        with torch.no_grad(): #전체 Acc, IoU 계산
            mean_acc, class_acc = self.metrics.getAcc()
            mean_recall, class_recall = self.metrics.getRecall()
            mean_iou, class_iou = self.metrics.getIoU()
